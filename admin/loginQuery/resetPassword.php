...
$sql=mysqli_query($conn,"select * from tbl_admin where email='$email'") or die(mysqli_query($conn));
...
// $pass =md5( $_REQUEST['txtpassword']);
			/*-----------------------------------------------------------------------*/
	$sqli = mysqli_query($conn,"update tbl_admin set  password = md5('$pass') where id = '$userID'") or die(mysqli_error($conn));
