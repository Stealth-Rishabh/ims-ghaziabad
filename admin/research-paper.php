...
<?php
     ...
     if(isset($_GET['id']))
    {
        ...
        extract($row);
         $facultyidAry = explode(',',$faculty_id);
                ...
                <div class="panel-body">
                    <form role="form" id="form" action="dataQuery/process.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="txtselyear" class="js-source-states form-control">
                          ...
                          <div class="form-group">
                            <label>Select IMS Faculty *</label> 
                            <select class="js-source-states-2 form-control" multiple="multiple" name="selfaculty[]">
                            <option value="">Select</option>
                            <?php 
                            $sqlfaculty = mysqli_query($conn, "SELECT * FROM tbl_faculty_member ORDER BY name") or die(mysqli_error($conn));
                            while ($rowfaculty = mysqli_fetch_assoc($sqlfaculty)) {
                                // Check if the current faculty ID exists in $facultyidAry to mark it as selected
                                $selected = (!empty($facultyidAry) && in_array($rowfaculty['id'], $facultyidAry)) ? "selected" : "";
                            ?>
                                <option value="<?php echo $rowfaculty['id']; ?>" <?php echo $selected; ?>>
                                    <?php echo $rowfaculty['name']; ?>
                                </option>
                            <?php } ?>
                        </select>
                        ...
                        <div class="form-group">
                            <label>Other Faculty Name</label> 
                            <input type="text" class="form-control" name="txtotherfaculty" value="<?php echo $other_faculty ;?>">
                        </div> 
                        <div class="form-group">
