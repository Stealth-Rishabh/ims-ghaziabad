    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="hpanel">
                ...
                <?php  
                   $i=1;
                   $sql=mysqli_query($conn,"select * from tbl_hospital_detail where type='Hospital' ORDER BY id desc")or die(mysqli_error());
                while($rowpic=mysqli_fetch_assoc($sql))
                {extract($rowpic);
                    $hospitaliddd=$rowpic['hospitalid'];
                    $sqlhospitaldoctor=mysqli_query($conn,"select * from tbl_hospital_doctor where hospital_id='$hospitaliddd'")or die(mysqli_error());
                    $doctorcount=mysqli_num_rows($sqlhospitaldoctor); 
                ?>
                    <tr>
                        <td><?php echo $i++;?></td>
                        <td><img height="50" width="50" src="../media/hospitalpic/<?php echo $rowpic['image'];?>"></td> 
                        <td><?php echo $name;?><br><b>HOP ID : </b><?php echo $hospitalid;?></td>
                        <td><?php echo $profiledate;?></td>
                        <td><?php echo $rowpic['address'];?><br><b>City :</b><?php echo $cityNameAry[$cityid];?></td>
                        <td><a href="hospital-doctor-list.php?hospitalid=<?php echo $hospitaliddd; ?>" class="btn-sm btn-primary"><?php echo $doctorcount; ?> Doctor <i class="fa fa-arrow-right"></i> </a></td>
                         <td>
                            <center>
                             <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a href="dataQuery/process.php?action3=activeHospital&id=<?php echo $rowpic['id'];?>">Active</a></li>
                                    <li><a href="dataQuery/process.php?action3=deactiveHospital&id=<?php echo $rowpic['id'];?>">De-Active</a></li>
                                </ul>
                            </div>
                            </center>
                         </td> 
                         <td><a class="demo3 btn-danger btn btn-sm" path="dataQuery/process.php?action2=<?php echo $deleteMode ?>&hospitalid=<?php echo $rowpic['hospitalid'];?>" ><i class="fa fa-trash"> </i> Delete Hospital</a></td>
